    ...
    public static Observable<ResponseBody> uploadFilesWithParams(String uploadUrl, String fileName, Map<String, Object> paramsMap, List<String> filePaths) {
        ...
        if (null != paramsMap) {
            for (String key : paramsMap.keySet()) {
                if(paramsMap.get(key) instanceof File){
                    RequestBody imageBody = RequestBody.create(MediaType.parse("multipart/form-data"), (File)paramsMap.get(key));
                    //"fileName"+i 后台接收图片流的参数名
                    builder.addFormDataPart(fileName, ((File)paramsMap.get(key)).getName(), imageBody);
                }else{
