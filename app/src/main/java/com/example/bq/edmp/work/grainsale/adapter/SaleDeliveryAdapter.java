...
import com.example.bq.edmp.word.put_warehouse.adapter.ChuListAdapter;
 ...
 */
public class SaleDeliveryAdapter extends RecyclerView.Adapter<SaleDeliveryAdapter.Holder> {

    private List<SaleDeliveryBean.DataBean.RowsBean> list;

    public SaleDeliveryAdapter(List<SaleDeliveryBean.DataBean.RowsBean> list) {
        this.list = list;
    }

    public void addMoreData(List<SaleDeliveryBean.DataBean.RowsBean> data) {
        if (data != null) {
    ...
    public void onBindViewHolder(@NonNull Holder holder, final int position) {
        final SaleDeliveryBean.DataBean.RowsBean rowsBean = list.get(position);
        ...
        List<SaleDeliveryBean.DataBean.RowsBean.StockSubItemsBean> stockAddItems = rowsBean.getStockSubItems();
        if (stockAddItems != null && stockAddItems.size() != 0) {
            ...
            NeiSaleDeliveryAdapter neiSaleDeliveryAdapter = new NeiSaleDeliveryAdapter(stockAddItems);
            holder.zi_rv.setLayoutManager(new LinearLayoutManager(ProApplication.getmContext()));
            holder.zi_rv.setAdapter(neiSaleDeliveryAdapter);
    ...
    public interface OnItemClickListener {
        void onItemClick(int pos, SaleDeliveryBean.DataBean.RowsBean rowsBean);
    }
