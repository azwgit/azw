 ...
 */
public class LibraryListAdapter extends RecyclerView.Adapter<LibraryListAdapter.Holder> {

    private List<LibraryBean.DataBean.RowsBean> list;
    private String TYPE;


    public LibraryListAdapter(List<LibraryBean.DataBean.RowsBean> list, String TYPE) {
        this.list = list;
        this.TYPE = TYPE;
    }

    public void addMoreData(List<LibraryBean.DataBean.RowsBean> data) {
        if (data != null) {
    ...
    public void onBindViewHolder(@NonNull Holder holder, final int position) {
        final LibraryBean.DataBean.RowsBean rowsBean = list.get(position);
        ...
        List<LibraryBean.DataBean.RowsBean.StockAddItemsBean> stockAddItems = rowsBean.getStockAddItems();
        if (stockAddItems != null && stockAddItems.size() != 0) {
            ...
            RuNeiListAdapter ruNeiListAdapter = new RuNeiListAdapter(stockAddItems);
            holder.zi_rv.setLayoutManager(new LinearLayoutManager(ProApplication.getmContext()));
            holder.zi_rv.setAdapter(ruNeiListAdapter);
        } else {
    ...
    public interface OnItemClickListener {
        void onItemClick(int pos, LibraryBean.DataBean.RowsBean rowsBean);
    }
