 ...
 * 申请支出报账详情
 */
public class CopyTravelDetailAct extends BaseTitleActivity {

    public static Intent newIntent(Context context, String id) {
        Intent intent = new Intent(context, CopyTravelDetailAct.class);
    ...
    protected void initData() {
        ...
        mOtherAdapter.setOnClickLisenter(new OtherExpensesAdp.OnClickLisenter() {
            ...
            public void onClick(int position, PayInfoBean bean) {
                if(bean.getImg_list().size()<=0){
                    //添加其他费用
                    startActivityForResult(OtherExpensesAct.newIntent(CopyTravelDetailAct.this, bean.getDesc(),position,dataBean.getData().getId()+""), CITY_CAR_MONEY_CODE);
                }else{
                    //修改其他费用
        ...
        mOtherAdapter.setOnPicLisenter(new PayInfoAdp.OnPicLisenter() {
            ...
            public void onPicClick(int themeId, int position, List<LocalMedia> selectList) {
                PictureSelector.create(CopyTravelDetailAct.this).themeStyle(themeId).openExternalPreview(position, selectList);
            }
        });
...
//        txtTabTitle.setText("详情");
    ...
    private void getRembursementDetails(String id) {
        String a = MD5Util.encode("title=" + id);
