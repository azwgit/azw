...
public class ReturnManagementActivityListAdp extends RecyclerView.Adapter<ReturnManagementActivityListAdp.Holder> {

    private List<ReturnsManagementListBean.DataBean.RowsBean> list;

    public ReturnManagementActivityListAdp(List<ReturnsManagementListBean.DataBean.RowsBean> list) {
        this.list = list;
    }

    public void addMoreData(List<ReturnsManagementListBean.DataBean.RowsBean> data) {
        if (data != null) {
    ...
    @NonNull
    @Override
    public ReturnManagementActivityListAdp.Holder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new Holder(LayoutInflater.from(ProApplication.getmContext()).inflate(R.layout.item_returns_activity_list, null));
    }

    @Override
    public void onBindViewHolder(@NonNull ReturnManagementActivityListAdp.Holder holder, final int position) {
        final ReturnsManagementListBean.DataBean.RowsBean rowsBean = list.get(position);
        if (mItemClickListener != null) {
            ...
            holder.tv_number.setText("数量 " + MoneyUtils.formatMoney(rowsBean.getReturnQty()) + "公斤");
    ...
    protected ReturnManagementActivityListAdp.OnItemClickListener mItemClickListener;

    public interface OnItemClickListener {
        void onItemClick(int pos, ReturnsManagementListBean.DataBean.RowsBean rowsBean);
    }

    public void setOnItemClickListener(ReturnManagementActivityListAdp.OnItemClickListener listener) {
