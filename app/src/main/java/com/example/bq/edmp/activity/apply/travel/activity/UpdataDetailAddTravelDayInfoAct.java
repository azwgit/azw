 ...
 * 添加日程信息（差旅）
 */
public class UpdataDetailAddTravelDayInfoAct extends BaseTitleActivity {

    public static Intent newIntent(Context context, IntetnCode intetnCode) {
        Intent intent = new Intent(context, UpdataDetailAddTravelDayInfoAct.class);
    ...
    protected void initData() {
        ...
        setStartTime();
        setEndTime();
        getTravelInfo(intetnCode.getIdx()+"",intetnCode.getId()+"");
                ...
                if (newSelectList.size() > 0) {
                    ...
                    switch (mediaType) {
                        case 1:
                            ...
                            PictureSelector.create(UpdataDetailAddTravelDayInfoAct.this).themeStyle(themeId).openExternalPreview(position, newSelectList);
                            break;
                        case 2:
                            // 预览视频
                            PictureSelector.create(UpdataDetailAddTravelDayInfoAct.this).externalPictureVideo(media.getPath());
                            break;
                        case 3:
                            // 预览音频
                            PictureSelector.create(UpdataDetailAddTravelDayInfoAct.this).externalPictureAudio(media.getPath());
                            break;
                    }
                }
            }
        ...
        mAdapter.setOnDelterImg(new DeleteGridImageAdapter.DeleteImg() {
            ...
            public void deleteImgList(int postion) {
                ischeck=true;
                deleteImag(travelBean.getData().getReimburserItemBills().get(postion).getId()+"",postion,"2",intetnCode.getId()+"");
            }
        });
        // 清空图片缓存，包括裁剪、压缩后的图片 注意:必须要在上传完成后调用 必须要获取权限
        RxPermissions permissions = new RxPermissions(UpdataDetailAddTravelDayInfoAct.this);
    ...
    private DeleteGridImageAdapter.onAddPicClickListener onAddPicClickListener = new DeleteGridImageAdapter.onAddPicClickListener() {
        ...
        public void onAddPicClick() {
            ...
            if (mode) {
                ...
                PictureSelector.create(UpdataDetailAddTravelDayInfoAct.this)
            ...
            case RESULT_OK:
                switch (requestCode) {
                    case PictureConfig.CHOOSE_REQUEST:
                        ...
                        //TODO 上传图片接口
                        uploadImg(intetnCode.getIdx()+"","2",intetnCode.getId()+"");
                        break;
                }
                break;
        }
