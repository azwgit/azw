...
public class PurchaseListScanActivity extends BaseTitleActivity implements QRCodeView.Delegate{


    private static final String TAG = PurchaseListScanActivity.class.getSimpleName();
...
//        mZXingView.startCamera(Camera.CameraInfo.CAMERA_FACING_FRONT); // 打开前置摄像头开始预览，但是并未开始识别
    ...
    public void onScanQRCodeSuccess(String result) {
        ...
        if ("收购记录".equals(title)) {
            setResult(350, intent);
        }else if ("入库查询".equals(title)){
            setResult(450, intent);
        }else if ("出库查询".equals(title)){
            setResult(450, intent);
        }
        finish();
