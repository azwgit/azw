...
import com.example.bq.edmp.work.grainsale.adapter.Sale_Warehouse_Adapter;
 ...
 * 销售入库
 * */
public class SaleWarehousingActivity extends BaseTitleActivity {
    ...
    private ArrayList<SaleWarehouseBean.DataBean.RowsBean> rowsBeans;
    private Sale_Warehouse_Adapter sale_warehouse_adapter;
    ...
    protected int getLayoutId() {
        return R.layout.activity_sale_warehousing;
    }

    @Override
    protected void initView() {
        ProApplication.getinstance().addActivity(SaleWarehousingActivity.this);
        txtTabTitle.setText("入库记录");
        loadingDialog = new LoadingDialog(this);

        //入库数据
        rowsBeans = new ArrayList<>();
        sale_warehouse_adapter = new Sale_Warehouse_Adapter(rowsBeans);
        xr.setAdapter(sale_warehouse_adapter);
        xr.setPullRefreshEnabled(true);
        xr.setLoadingMoreEnabled(true);
        xr.setLayoutManager(new LinearLayoutManager(SaleWarehousingActivity.this));
        ...
        sale_warehouse_adapter.setOnItemClickListener(new Sale_Warehouse_Adapter.OnItemClickListener() {
            @Override
            public void onItemClick(int pos, SaleWarehouseBean.DataBean.RowsBean rowsBean) {
                //入庫詳情
                Intent intent = new Intent(SaleWarehousingActivity.this, SaleWarehousingDetailsActivity.class);
        ...
        recyclerView.setLayoutManager(new GridLayoutManager(SaleWarehousingActivity.this, 3));
    ...
    private void gainData() {
        ...
        RxHttpUtils.createApi(SaleApi.class)
                ...
                .compose(Transformer.<SaleWarehouseBean>switchSchedulers(loadingDialog))
                .subscribe(new NewCommonObserver<SaleWarehouseBean>() {
                    @Override
                    ...
                    @Override
                    protected void onSuccess(SaleWarehouseBean saleWarehouseBean) {
                        List<SaleWarehouseBean.DataBean.RowsBean> rows = saleWarehouseBean.getData().getRows();
                        if (rows != null && rows.size() != 0) {
                            ...
                            sale_warehouse_adapter.notifyDataSetChanged();
                        } else {
                ...
                .subscribe(new NewCommonObserver<SaleWarehouseBean>() {
                    ...
                    protected void onSuccess(SaleWarehouseBean saleWarehouseBean) {
                        ...
                        if (rows != null && rows.size() != 0) {
                            ...
                            rowsBeans.addAll(rows);
                            sale_warehouse_adapter.addMoreData(rows);
                        } else {
                            ToastUtil.setToast("暂无数据");
                            xr.setNoMore(true);
                        }
...
//      lp.alpha = 9f; // 透明度
        ...
        buttom_rv.setLayoutManager(new LinearLayoutManager(SaleWarehousingActivity.this));
    ...
    protected void onDestroy() {
        super.onDestroy();
        ProApplication.getinstance().finishActivity(SaleWarehousingActivity.this);
    }
}
